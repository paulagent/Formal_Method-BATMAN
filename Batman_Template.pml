...
/* INSERT INITIAL RECEIVING BUFFER */
...
loop:	do
	:: true ->
		if
		/* INSERT RANDOM PICK FIRST NEIGHBOR */
		fi;
...
reloop:		do
		:: true ->
			if
			/* INSERT RANDOM PICK SECOND NEIGHBOR * /
			fi;
...
repeat:	do
	:: true ->
		if
		/* INSERT RANDOM SELECT FAIL NODE */
		fi;
		int q;
...
inline generatePacket() {
	...
	if
	/* INSERT RANDOM SELECT PACKET SOURCE */
	fi;

	do
	:: true ->
		if
		/* INSERT RANDOM SELECT PACKET DESTINATION */
		fi;
 ...
 */
inline setNeighbors() {
	/* INSERT INITIAL NEIGHBOR SETTING */
}
...
inline setInitSeqNums() {
	/* INSERT RANDOM ASSIGN SEQUENCE NUMNER */
}
